/**
 * Temperature-SOC-Power Worker v1.2
 * Fixed entity names for power sensors
 */
...
// Get Power history for a specific date - FIXED entity names
async function getPowerHistory(deviceId, date) {
  const deviceLower = deviceId.toLowerCase();
  
  // Correct entity names based on HA
  const entities = [
  ...
  // Process each entity - map to friendly names
  const result = {};
