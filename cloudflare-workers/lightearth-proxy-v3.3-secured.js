/**
 * Lightearth Proxy Worker v3.3-secured
 ...
 * - Security headers
 * - **NEW**: /api/cloud/* endpoints require same-origin or API key
 * - **NEW**: Direct browser access to /api/cloud/* blocked
 * 
 * Environment Variables needed:
 * - PI_URL: Private Infrastructure URL
 * - PI_TOKEN: Private Infrastructure Access Token
 * - API_SECRET_KEY: (Optional) API key for direct access
 */
...
const SECURITY_CONFIG = {
  ...
  // Allowed origins for /api/cloud/* endpoints (same-origin protection)
  allowedOrigins: [
...
function isOriginAllowed(origin) {
  if (!origin) return false; // Require origin for cloud endpoints
...
// Check if request is from same-origin (web app AJAX)
function isSameOriginRequest(request) {
  const origin = request.headers.get('Origin');
  const referer = request.headers.get('Referer');
  const secFetchSite = request.headers.get('Sec-Fetch-Site');
  
  // Check Sec-Fetch-Site header (set by browsers for fetch/XHR)
...
export default {
  async fetch(request, env) {
    ...
    // ========== PROTECT /api/cloud/* ENDPOINTS ==========
    // Block direct browser access, only allow from web app or with API key
    if (path.startsWith('/api/cloud/')) {
      const hasApiKey = hasValidApiKey(request, env);
      const isSameOrigin = isSameOriginRequest(request);
      
      if (!hasApiKey && !isSameOrigin) {
        console.log(`[BLOCKED] Direct access to ${path} from ${clientIP} - No API key or same-origin`);
        return new Response(JSON.stringify({ 
          error: 'Direct access not allowed. Please use the web application.',
          code: 'FORBIDDEN',
          hint: 'This endpoint is protected. Access it through the web app or provide a valid API key.'
        }), { status: 403, headers });
      }
    }
    ...
    // Health check - minimal info, no endpoint list
    if (path === '/' || path === '/health') {
      return new Response(JSON.stringify({
        status: 'ok',
        version: '3.3-secured',
        ...
        security: {
          ...
          corsProtected: true,
          cloudApiProtected: true
        }
        // Removed endpoints list for security
      }), { headers });
    }

    // ============ CLOUD INFRASTRUCTURE ENDPOINTS (PROTECTED) ============
