/**
 * Temperature-SOC-Power Worker v2.4
 * - NEW: /api/ha/statistics/{deviceId}/year?year=YYYY - Yearly Statistics (replaces Railway API)
 * - /api/solar/dashboard/{deviceId} - Solar Dashboard from sensor.device_*_pv_year attributes
...
// ========================================
// NEW v2.4: /api/ha/statistics/{deviceId}/year - Yearly Statistics
// Replaces Railway API: /api/ha/statistics/{deviceId}/year?year=YYYY
// ========================================
async function getYearlyStatistics(deviceId, year) {
  const deviceLower = deviceId.toLowerCase();
  ...
  try {
    ...
    // All monthly arrays from sensor attributes
    ...
    console.error('Error fetching yearly statistics:', e.message);
    return {
      success: false,
      deviceId: deviceUpper,
      year: year,
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      // NEW v2.4: Yearly Statistics - /api/ha/statistics/{deviceId}/year
      // Replaces Railway: /api/ha/statistics/{deviceId}/year?year=YYYY
      const yearlyStatsMatch = path.match(/^\/api\/ha\/statistics\/([^\/]+)\/year$/);
      if (yearlyStatsMatch) {
        const deviceId = yearlyStatsMatch[1];
        const year = parseInt(url.searchParams.get('year')) || new Date().getFullYear();
        return jsonResponse(await getYearlyStatistics(deviceId, year), origin);
      }
